...
import {
  ...
  detectDuplicateMints,
  canOnlyAddOneToken,
  validMintAddress,
  noEditsToPreviousLinesAllowed,
  isCommunityValidated,
  isSymbolConfusing,
  newTokensHaveMatchingOnchainMeta,
  findAddedTokens,
} from "./utils/validate";
...
import {
  ...
  allowedNotCommunityValidated,
} from "./utils/duplicate-symbols";

export async function validateValidatedTokensCsv(
  filename: string
): Promise<number> {
  ...
  doubleCheckMetadataOnChain = await newTokensHaveMatchingOnchainMeta(
    connection,
    newTokens
  );
  ...
  console.log(
    ...
    duplicateSymbols,
    `(${allowedDuplicateSymbols.length} exceptions)`
  );
  ...
  console.log(
    "Not Community Validated:",
    notCommunityValidated,
    `(${allowedNotCommunityValidated.length} exceptions)`
  );
  ...
  return (
    duplicateSymbols +
    duplicateMints +
    attemptsToAddMultipleTokens +
    invalidMintAddresses +
    noEditsAllowed +
    doubleCheckMetadataOnChain
  );
}
